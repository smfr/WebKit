    ...
    BackgroundBleedAvoidance bleedAvoidance, const InlineIterator::InlineBoxIterator& inlineBoxIterator, const LayoutRect& backgroundImageStrip, CompositeOperator op, RenderElement* backgroundObject, BaseBackgroundColorUsage baseBgColorUsage) const
{
    ...
    auto borderShapeRespectingBleedAvoidance = [&](bool shrinkForBleedAvoidance = true) {
    ...
    if (!isRoot && !clippedWithLocalScrolling && !shouldPaintBackgroundImage && isBorderFill && !bgLayer.next()) {
        ...
        if (hasRoundedBorder && bleedAvoidance != BackgroundBleedUseTransparencyLayer) {
            auto borderShape = borderShapeRespectingBleedAvoidance();
...
//            } else {
//                WTF_ALWAYS_LOG("When does this happen?");
//                context.save();
//
//                // FIXME: But we know we're renderable.
    ...
    for (const ShadowData* shadow = style.boxShadow(); shadow; shadow = shadow->next()) {
        ...
        if (shadow->style() == ShadowStyle::Normal) {
            ...
            if (hasBorderRadius) {
                ...
                if (influenceShape.encloses(m_paintInfo.rect))
            ...
            // Inset shadow.
            auto holeRect = borderShape.innerRect();
            holeRect.inflate(-shadowSpread);


            // FIXME: Need to do this math before the spread inset.
            ...
            auto holeShape = BorderShape::shapeForBorderRect(style, holeRect, borderAndSpreadWidths, includeLogicalLeftEdge, includeLogicalRightEdge);
            if (holeShape.isEmpty()) {
            ...
            holeShape.fillRectWithInnerHoleShape(context, shadowCastingRect, shadowColor.opaqueColor(), deviceScaleFactor);
        }
    }
}
